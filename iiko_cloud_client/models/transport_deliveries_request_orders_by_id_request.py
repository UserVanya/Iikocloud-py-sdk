...
class TransportDeliveriesRequestOrdersByIdRequest(BaseModel):
    ...
    order_ids: Optional[List[StrictStr]] = Field(default=None, description="IDs of orders information on which is required.                > Required if \"posOrderIds\" is null. Must be null if \"posOrderIds\" is not null.                > Maximum allowed \"orderIds\" to request - `200`.    The guaranteed order availability period is the last 7 days. To access earlier orders, use the `/api/1/deliveries/history/by_delivery_date_and_phone` method.", alias="orderIds")
    source_keys: Optional[List[StrictStr]] = Field(default=None, description="Source keys.", alias="sourceKeys")
    pos_order_ids: Optional[List[StrictStr]] = Field(default=None, description="POS order IDs information on which is required.                > Required if \"orderIds\" is null. Must be null if \"orderIds\" is not null.                > Maximum allowed \"posOrderIds\" to request - `200`.    The guaranteed order availability period is the last 7 days. To access earlier orders, use the `/api/1/deliveries/history/by_delivery_date_and_phone` method.", alias="posOrderIds")
