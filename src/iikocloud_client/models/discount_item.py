...
class DiscountItem(BaseModel):
    ...
    Discount.
    """ # noqa: E501
    discount_type: DiscountType = Field(description="Discount type.                 Can be obtained by `/api/1/discounts` operation.", alias="discountType")
    selective_positions: Optional[List[UUID]] = Field(default=None, description="Order item positions.", alias="selectivePositions")
    selective_positions_with_sum: Optional[List[PositionWithSum]] = Field(default=None, description="Order item positions with position discount sum.   > Allowed from version `8.5.6`.", alias="selectivePositionsWithSum")
    sum: Union[StrictFloat, StrictInt] = Field(description="Total.")
    __properties: ClassVar[List[str]] = ["discountType", "selectivePositions", "selectivePositionsWithSum", "sum"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "discountType": DiscountType.from_dict(obj["discountType"]) if obj.get("discountType") is not None else None,
            "selectivePositions": obj.get("selectivePositions"),
            "selectivePositionsWithSum": [PositionWithSum.from_dict(_item) for _item in obj["selectivePositionsWithSum"]] if obj.get("selectivePositionsWithSum") is not None else None,
            "sum": obj.get("sum")
        })
        return _obj
