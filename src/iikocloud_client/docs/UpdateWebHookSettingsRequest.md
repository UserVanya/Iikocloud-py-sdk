# UpdateWebHookSettingsRequest

Request to add or update webhooks settings for listed api logins of the specified organization.

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**auth_token** | **str** | Authorization token to pass to the webhook handler. | [optional] 
**organization_id** | **UUID** | Organization UOC Id.                Can be obtained by &#x60;/api/1/organizations&#x60; operation. | 
**web_hooks_filter** | [**WebHooksFilter**](WebHooksFilter.md) | Webhooks filter. | [optional] 
**web_hooks_uri** | **str** | Webhook handler url. | 

## Example

```python
from iikocloud_client.models.update_web_hook_settings_request import UpdateWebHookSettingsRequest

# TODO update the JSON string below
json = "{}"
# create an instance of UpdateWebHookSettingsRequest from a JSON string
update_web_hook_settings_request_instance = UpdateWebHookSettingsRequest.from_json(json)
# print the JSON string representation of the object
print(UpdateWebHookSettingsRequest.to_json())

# convert the object into a dict
update_web_hook_settings_request_dict = update_web_hook_settings_request_instance.to_dict()
# create an instance of UpdateWebHookSettingsRequest from a dict
update_web_hook_settings_request_from_dict = UpdateWebHookSettingsRequest.from_dict(update_web_hook_settings_request_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


