...
def validate_private_regular_file(path: Path, *, label: str) -> os.stat_result:
    ...
    if stat.S_IMODE(current.st_mode) != _FILE_MODE:
        raise SafetyError(
            f"{label} must have mode 0600, got {_mode_text(current.st_mode)}: {path}"
        )
    if current.st_uid != os.getuid():
        raise SafetyError(f"{label} is not owned by the current user: {path}")
    if current.st_nlink != 1:
        raise SafetyError(f"{label} must not have multiple hard links: {path}")
    return current
