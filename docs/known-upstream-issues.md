...
| `counter-enums-string-1-*`, `counter-enums-string-2-*` | `iikoNet.Common.Enums.CounterMetric`, `iikoNet.Common.Enums.CounterPeriod` | Заменить integer enums на string enums с live-проверенными наборами значений | `tests/contracts/test_counter_enums.py`; guarded live-probes 2026-07-23 | 2026-07-23 | Upstream объявляет string enums с теми же наборами значений |
...
| `external-menu-schema-ExternalMenuItem-properties-taxCategory{,-items-remove,-type-remove}` | `ExternalMenuItem.properties.taxCategory`: object ошибочно описан как array | Nullable `oneOf` со ссылкой на `TaxCategoryDto` | Versioned external-menu fixtures и schema repair tests | 2026-07-20 | Upstream содержит корректный nullable ref |
...
| `external-menu-schema-ExternalMenuV{3,4}-properties-overrideTaxCategories{,-items-remove}` | `ExternalMenuV3/V4.properties.overrideTaxCategories`: UUID-keyed map ошибочно описан как array | Object с `additionalProperties`, значением которого является array `OverrideTaxesDto` (с upstream 9.8.6.1 общий для V3 и V4) | `external-menu-v3.json`, `external-menu-v4.json` | 2026-07-20 | Upstream содержит корректную map schema |
